...
sessions_features<-sessions_features[,c(1:392)]
# load data
...
## Taking only the ones who have history
...
trainIndex<-generatefolds(y,0.5,11)
...
dtrain<-xgb.DMatrix(data=data.matrix(X_train[,-c(1,3)]),label=y)
...
xgb.cv(params=param,nrounds=300,data=dtrain,nfold=5,
       #feval=evalerror,
       verbose=TRUE,
       showsd=FALSE,
       print.every.n = 2,
...
write.csv(predictions, "hist/xgboost_j1.csv", quote=FALSE, row.names = FALSE)
...
y_pred <- predict(xgb, data.matrix(X_test[,-c(1,3)]),ntree=520) #ntreelimit=500
...
predictions<-cbind(id,predictions)
predictions<-predictions[order(id),]
write.csv(predictions, "hist/xgboost_k4.csv", quote=FALSE, row.names = FALSE)
